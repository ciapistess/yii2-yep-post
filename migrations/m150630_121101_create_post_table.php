...
class m150630_121101_create_post_table extends Migration
{
    ...
    public function safeUp()
    {
        ...
        $RESTRICT_OR = 'RESTRICT';
        $CASCADE_OR = 'CASCADE';
        $SETNULL_OR = 'SET NULL';
        if ($this->db->driverName === 'mssql' || $this->db->driverName === 'sqlsrv' || $this->db->driverName === 'dblib') {
          $RESTRICT_OR = 'NO ACTION'; // 'NO ACTION' is equivalent to 'RESTRICT' in MSSQL
          $CASCADE_OR = 'NO ACTION';
          $SETNULL_OR = 'NO ACTION';
        }
        ...
        $this->addForeignKey('fk_post_category_updated_by', self::POST_CATEGORY_TABLE, 'updated_by', '{{%user}}', 'id', $SETNULL_OR, $CASCADE_OR);
        ...
        $this->addForeignKey('fk_page_created_by', self::POST_TABLE, 'created_by', '{{%user}}', 'id', $SETNULL_OR, $CASCADE_OR);
        // $this->addForeignKey('fk_page_updated_by', self::POST_TABLE, 'updated_by', '{{%user}}', 'id', 'SET NULL', 'CASCADE');
        $this->addForeignKey('fk_page_updated_by', self::POST_TABLE, 'updated_by', '{{%user}}', 'id', $SETNULL_OR, $CASCADE_OR);
